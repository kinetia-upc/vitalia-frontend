...
const passwordsMatch = computed(() => form.password === form.repeatPassword);
...
async function submit() {
    ...
    await router.replace(user.role === "patient" ? "/patient/dashboard" : `/${user.role}/dashboard`);
}
</script>
...
<template>
  <main class="auth-page auth-page-wide">
    <section class="auth-card auth-card-wide" aria-labelledby="signup-title">
      ...
      <form class="auth-grid-form" @submit.prevent="submit">
        ...
        <label class="auth-field">
          ...
          <input v-model.trim="form.department" class="auth-plain-input" required />
        </label>

        <label class="auth-field">
          <span>Province</span>
          <input v-model.trim="form.province" class="auth-plain-input" required />
        </label>

        <label class="auth-field">
          <span>District</span>
          <input v-model.trim="form.district" class="auth-plain-input" required />
        </label>
        ...
        <div class="auth-register-actions">
          ...
          <button class="auth-submit auth-submit-compact" type="submit" :disabled="authStore.loading || !passwordsMatch">
    ...
    <footer class="auth-footer">
      <span>© 2026 VITALIA. ALL RIGHTS RESERVED.</span>
      <nav>
